...
namespace srot {
	...
	template<class _Ty1, class _Ty2>
	struct Value2 {
		Value2(_Ty1 f, _Ty2 s) : first(f), second(s) {
		}
		Value2(ValueDelegate<_Ty1, _Ty2> &&r);
		Value2 &operator=(ValueDelegate<_Ty1, _Ty2> &&r);
		_Ty1 first;
		_Ty2 second;
	};
	...
	template<class _Ty1, class _Ty2>
	struct ValueDelegate {
		ValueDelegate(_Ty1 &f, _Ty2 &s) : _First(f), _Second(s) {
		}

		ValueDelegate(Value2<_Ty1, _Ty2> && v) : _First(std::move(v.first)), _Second(std::move(v.second)) {
		}

		ValueDelegate &operator=(Value2<_Ty1, _Ty2> && v) {
			_First = std::move(v.first);
			_Second = std::move(v.second);
		}

		ValueDelegate &operator=(ValueDelegate<_Ty1, _Ty2> &&v) {
			_First = std::move(v._First);
			_Second = std::move(v._Second);
		}

		ValueDelegate &operator=(ValueDelegate<_Ty1, _Ty2> const &v) {
			_First = v._First;
			_Second = v._Second;
		}
	...
	template<class _Ty1, class _Ty2>
	Value2<_Ty1, _Ty2> &Value2<_Ty1, _Ty2>::operator=(ValueDelegate<_Ty1, _Ty2> &&r) {
		first = std::move(r._First);
		second = std::move(r._Second);
	}

	template<class _Ty1, class _Ty2>
	Value2<_Ty1, _Ty2>::Value2(ValueDelegate<_Ty1, _Ty2> &&r) : first(std::move(r._First)), second(std::move(r._Second)) {
	}
	...
	struct ValueDelegateCompare {
		...
		template<class _Ty1, class _Ty2>
		bool operator()(ValueDelegate<_Ty1, _Ty2> &l, ValueDelegate<_Ty1, _Ty2> &r) const {
			return _pred(l._First, r._First);
		}

		template<class _Ty1, class _Ty2>
		bool operator()(Value2<_Ty1, _Ty2> &l, ValueDelegate<_Ty1, _Ty2> &r) const {
			return _pred(l.first, r._First);
		}

		template<class _Ty1, class _Ty2>
		bool operator()(ValueDelegate<_Ty1, _Ty2> &l, Value2<_Ty1, _Ty2> &r) const {
			return _pred(l._First, r.first);
		}
	...
	struct BindIter {
		...
		typedef ValueDelegate<typename std::iterator_traits<_RanIt>::value_type,
			typename std::iterator_traits<_RanIt1>::value_type> delegate_type;
...
namespace sort {
	...
	{	// order [_First, _Last), using operator<
		...
		BindIter<_RanIt, _RanIt1> begin(_First, _Last);
		BindIter<_RanIt, _RanIt1> end(_First1, _First1 + std::iterator_traits<_RanIt>::difference_type(_Last - _First));
